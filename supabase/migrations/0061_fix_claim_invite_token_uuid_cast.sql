-- 0061_fix_claim_invite_token_uuid_cast.sql
--
-- Sentry EQ-CARDS-T: eq_cards_claim_invite crashes with
--   ServerFailure(42883): operator does not exist: uuid = text
--
-- worker_invites.token is type uuid but p_token is declared as text.
-- The comparison `WHERE token = p_token` fails because PostgreSQL has no
-- implicit cast for uuid = text. Fix: cast the parameter at the call site.
...
BEGIN
  ...
  SELECT * INTO v_invite FROM public.worker_invites WHERE token = p_token::uuid FOR UPDATE;
