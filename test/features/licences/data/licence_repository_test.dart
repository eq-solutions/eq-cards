...
void main() {
  ...
  group('licenceToUpsertPayload', () {
    test('user_id comes from the parameter, not from the model', () {
      // The parameter is the authenticated session id; RLS would reject
      // a payload whose user_id didn't match auth.uid(), so the contract
      // is explicit.
      final payload = licenceToUpsertPayload(base(), 'authed-user-1');
      expect(payload['user_id'], 'authed-user-1');
    });

    test('omits id for new licences (insert path)', () {
      final payload = licenceToUpsertPayload(base(), 'u1');
      expect(payload.containsKey('id'), false);
    });

    test('includes id for existing licences (update path)', () {
      final payload = licenceToUpsertPayload(base(id: 'lic-1'), 'u1');
    ...
    test('pads single-digit months and days', () {
      ...
      final payload = licenceToUpsertPayload(l, 'u1');
    ...
    test('includes optional fields when set', () {
      final l = Licence(
        ...
        state: 'NSW',
        photoFrontPath: 'u1/lic-1/front.jpg',
        photoBackPath: 'u1/lic-1/back.jpg',
        notes: 'Class C',
      );
      final payload = licenceToUpsertPayload(l, 'u1');
